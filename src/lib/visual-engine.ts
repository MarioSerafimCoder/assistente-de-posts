...
.verti-outline{position:absolute;z-index:1;width:${output === "feed" ? 720 : 760}px;height:auto;right:${output === "feed" ? -165 : -275}px;${output === "feed" ? "bottom:-310px" : "top:-82px"};}
.verti-headline{position:absolute;z-index:4;left:${output === "feed" ? 78 : 92}px;right:${output === "feed" ? 120 : 100}px;top:${output === "feed" ? 145 : 238}px;margin:0;font-family:'${brand.typography.headline.family}',${brand.typography.headline.fallback};font-size:${headlineFontSize}px;line-height:.96;letter-spacing:-.045em;font-weight:800;}
...
.verti-callout{position:absolute;z-index:5;left:${output === "feed" ? 78 : 68}px;top:${output === "feed" ? 858 : 1364}px;width:${output === "feed" ? 565 : 650}px;min-height:${output === "feed" ? 104 : 118}px;box-sizing:border-box;border-radius:24px;background:${brand.colors.light};color:${brand.colors.primary};display:flex;align-items:center;justify-content:center;padding:${output === "feed" ? "20px 22px" : "22px 26px"};font-size:${Math.max(18, Math.min(output === "feed" ? 25 : 27, bodyFit.fontSize))}px;line-height:1.35;font-weight:400;box-shadow:0 8px 30px rgba(0,0,0,.12);}
.verti-callout-text{display:block;width:100%;margin:auto;text-align:left}
