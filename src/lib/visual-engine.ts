...
function renderVertiOption01({ brand, slide, output, template, variant }: {
  ...
  const marker = slide.content.cta ? "↗" : "K";
...
.verti-callout{position:absolute;z-index:5;left:${output === "feed" ? 78 : 68}px;top:${output === "feed" ? 858 : 1364}px;width:${output === "feed" ? 565 : 650}px;min-height:${output === "feed" ? 104 : 118}px;box-sizing:border-box;border-radius:24px;background:${brand.colors.light};color:${brand.colors.primary};display:flex;align-items:center;gap:20px;padding:${output === "feed" ? "20px 22px" : "22px 26px"};font-size:${Math.max(18, Math.min(output === "feed" ? 25 : 27, bodyFit.fontSize))}px;line-height:1.15;font-weight:200;box-shadow:0 8px 30px rgba(0,0,0,.12);}
.verti-callout-text{flex:1}.verti-marker{display:grid;place-items:center;flex:0 0 auto;width:${output === "feed" ? 52 : 58}px;height:${output === "feed" ? 52 : 58}px;border:6px solid ${brand.colors.accent};border-radius:50%;color:${brand.colors.primary};font-family:Arial,sans-serif;font-size:25px;font-weight:900;}
