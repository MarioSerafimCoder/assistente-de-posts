...
export function BrandTemplatePreviews({ brand, templates }: { brand: BrandDefinition; templates: TemplateDefinition[] }) {
  return <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{templates.map((template, index) => {
    const isVerti = template.id === "verti-option-01";
    const slide: GenerationSlide = {
      id: template.id,
      order: index,
      content: isVerti
        ? { id: template.id, role: "cover", headline: "Iluminação em altura\nexige conhecimento técnico.", body: "Cada instalação precisa ser planejada para garantir segurança durante a execução e eficiência no resultado.", cta: "Saiba mais" }
        : { id: template.id, role: index === templates.length - 1 ? "cta" : "cover", headline: template.name, body: "Template demonstrativo pronto para receber o conteúdo estruturado.", cta: index === templates.length - 1 ? "Conheça mais" : undefined },
      templateId: template.id,
      alternatives: [],
      variant: isVerti ? "image" : index % 2 ? "light" : "dark",
      logoVariant: "auto",
      imageUrl: isVerti ? brand.images[0]?.path : undefined,
