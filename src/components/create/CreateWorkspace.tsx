...
import type { RequestedFormat } from "@/types/content";
...
export function CreateWorkspace() {
  ...
  const [config, setConfig] = useState<{ openaiConfigured: boolean; model: string } | null>(null);
  ...
  const canGenerate = copy.trim().length >= 3 && outputs.length > 0 && !loading;
  ...
  async function generate() {
    ...
    try {
      const structuredResponse = await fetch("/api/ai/structure-content", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ brandId, copy, requestedFormat: format, outputs, maxSlides, preserveCopy }),
      });
      const structured = await structuredResponse.json();
      if (!structuredResponse.ok) throw new Error(structured.error || "Não foi possível interpretar a copy.");
      const generationResponse = await fetch("/api/generations", {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ brandId, outputs, post: structured.post }),
      });
    ...
    <main className="mx-auto grid max-w-[1440px] grid-cols-1 gap-10 px-6 py-10 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-10">
      <section>
        <div className="mb-8 flex items-end justify-between gap-6">
          <div><p className="app-kicker">Nova criação</p><h1 className="mt-2 max-w-2xl text-4xl font-semibold tracking-[-.04em] md:text-5xl">Da copy à arte, sem rediagramar.</h1></div>
          {config && <span className={`hidden rounded-full border px-3 py-1.5 text-xs md:block ${config.openaiConfigured ? "border-emerald-400/25 bg-emerald-400/10 text-emerald-300" : "border-amber-400/25 bg-amber-400/10 text-amber-200"}`}>{config.openaiConfigured ? `OpenAI · ${config.model}` : "OpenAI não configurada"}</span>}
        </div>
        <div className="app-panel space-y-7">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <fieldset><legend className="app-label">Formato</legend><div className="app-segment mt-2">{(["auto", "single", "carousel"] as const).map((value) => <button type="button" key={value} onClick={() => setFormat(value)} className={format === value ? "active" : ""}>{value === "auto" ? "Automático" : value === "single" ? "Post único" : "Carrossel"}</button>)}</div></fieldset>
            <fieldset><legend className="app-label">Saídas</legend><div className="mt-2 flex gap-2">{(["feed", "story"] as const).map((output) => <button type="button" key={output} onClick={() => toggleOutput(output)} className={`app-check ${outputs.includes(output) ? "active" : ""}`}><span>{outputs.includes(output) && <Check size={13}/>}</span>{output === "feed" ? "Feed 4:5" : "Stories 9:16"}</button>)}</div></fieldset>
          </div>
          <div className="flex flex-wrap items-center justify-between gap-5 border-t border-white/10 pt-6">
            <label className="flex cursor-pointer items-center gap-3 text-sm"><input type="checkbox" className="sr-only" checked={preserveCopy} onChange={(event) => setPreserveCopy(event.target.checked)}/><span className={`app-switch ${preserveCopy ? "on" : ""}`}><span/></span><span><b className="block font-medium">Preservar minha copy</b><small className="text-white/40">{preserveCopy ? "Apenas hierarquia e cortes essenciais" : "Otimizar concisão e ritmo com IA"}</small></span></label>
            {format !== "single" && <label className="text-xs text-white/55">Máximo de slides <select className="ml-2 rounded-md border border-white/10 bg-white/5 px-2 py-1.5 text-white" value={maxSlides} onChange={(event) => setMaxSlides(Number(event.target.value))}>{[4,5,6,7,8,9,10].map((value) => <option key={value}>{value}</option>)}</select></label>}
          </div>
