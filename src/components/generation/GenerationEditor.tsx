...
export function GenerationEditor({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function restructure(requestedFormat: "auto" | "single" | "carousel") {
    ...
    setBusy(true); setError("");
    try {
      const response = await fetch("/api/ai/structure-content", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ brandId: generation.brandId, copy: generation.originalCopy, requestedFormat, outputs: generation.outputs, maxSlides: 8, preserveCopy: true }) });
      const data = await response.json(); if (!response.ok) throw new Error(data.error);
      const createdResponse = await fetch("/api/generations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ brandId: generation.brandId, outputs: generation.outputs, post: data.post }) });
